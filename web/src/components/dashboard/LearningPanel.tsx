...
import { GraduationCap, Bot, Loader2 } from "lucide-react";
...
export function LearningPanel() {
    ...
    ws.onmessage = (e) => {
      try {
        ...
        if (data.type === "agent:text" || data.type === "agent:thinking" || data.type === "agent:tool") {
          currentMsgsRef.current.push(data);
  ...
  const handleAnalyze = useCallback(() => {
    ...
    setHasResult(false);
    wsRef.current.send(JSON.stringify({ action: "learning", timeframe: selectedTimeframe }));
  }, [running, selectedTimeframe]);
